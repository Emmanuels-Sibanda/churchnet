...
const createTransporter = () => {
  // If SMTP not configured, return null (will log to console)
...
// Export same functions (reuse HTML templates from email.js)
// For simplicity, you can copy the template functions from email.js
// Or import and reuse them
...
// Reuse templates from main email service
const getBookingRequestToOwnerTemplate = (ownerName, bookingDetails) => {
...
const getBookingRequestToBookerTemplate = (bookerName, bookingDetails) => {
...
const getBookingApprovalTemplate = (bookerName, bookingDetails) => {
...
const getBookingRejectionTemplate = (bookerName, bookingDetails) => {
...
const sendBookingRequestToOwner = async (ownerEmail, ownerName, bookingDetails) => {
  const { subject, html } = getBookingRequestToOwnerTemplate(ownerName, bookingDetails);
...
const sendBookingRequestToBooker = async (bookerEmail, bookerName, bookingDetails) => {
  const { subject, html } = getBookingRequestToBookerTemplate(bookerName, bookingDetails);
  return await sendEmail(bookerEmail, subject, html);
};

const sendBookingApproval = async (bookerEmail, bookerName, bookingDetails) => {
  const { subject, html } = getBookingApprovalTemplate(bookerName, bookingDetails);
  return await sendEmail(bookerEmail, subject, html);
};

const sendBookingRejection = async (bookerEmail, bookerName, bookingDetails) => {
  const { subject, html } = getBookingRejectionTemplate(bookerName, bookingDetails);
  return await sendEmail(bookerEmail, subject, html);
};
